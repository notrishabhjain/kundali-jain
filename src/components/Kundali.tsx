...
const tabs: { id: MainTab; label: string; icon: React.ReactNode; color: string; bgLight: string }[] = [
  { id: 'vartaman', label: 'वर्तमान (Present)',       icon: <Compass className="w-5 h-5" />,  color: 'bg-amber-600',  bgLight: 'bg-amber-50 text-amber-700' },
  { id: 'kundali',  label: 'कुण्डली (Birth Chart)',    icon: <BarChart2 className="w-5 h-5" />, color: 'bg-indigo-600', bgLight: 'bg-indigo-50 text-indigo-700' },
  { id: 'karma',    label: 'कर्म (Karma)',              icon: <Scale className="w-5 h-5" />,     color: 'bg-rose-600',   bgLight: 'bg-rose-50 text-rose-700' },
  { id: 'remedies', label: 'उपाय (Remedies)',           icon: <Sparkles className="w-5 h-5" />,  color: 'bg-orange-600', bgLight: 'bg-orange-50 text-orange-700' },
  { id: 'dharma',   label: 'धर्म मार्ग (Dharma Path)', icon: <Map className="w-5 h-5" />,       color: 'bg-emerald-600',bgLight: 'bg-emerald-50 text-emerald-700' },
  { id: 'calendar', label: 'कैलेंडर (Calendar)',        icon: <Calendar className="w-5 h-5" />,  color: 'bg-blue-600',   bgLight: 'bg-blue-50 text-blue-700' },
  { id: 'tirtha',   label: 'तीर्थ यात्रा (Pilgrimage)', icon: <Mountain className="w-5 h-5" />,  color: 'bg-teal-600',   bgLight: 'bg-teal-50 text-teal-700' },
  { id: 'muhurta',  label: 'मुहूर्त (Timings)',          icon: <Clock className="w-5 h-5" />,     color: 'bg-cyan-600',   bgLight: 'bg-cyan-50 text-cyan-700' },
  { id: 'sadhana',  label: 'ट्रैकर (Dashboard)',        icon: <Target className="w-5 h-5" />,    color: 'bg-yellow-600', bgLight: 'bg-yellow-50 text-yellow-700' },
  { id: 'report',   label: 'रिपोर्ट (Report)',          icon: <Printer className="w-5 h-5" />,   color: 'bg-slate-600',  bgLight: 'bg-slate-50 text-slate-700' },
];
...
export default function Kundali({ profile }: KundaliProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Priority Banner */}
      <div className="bg-gradient-to-r from-orange-600 to-amber-600 text-white p-4 rounded-xl shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3">
          <div className="bg-white/20 p-2 rounded-full shrink-0">
            <AlertCircle className="w-6 h-6 text-white" />
          </div>
          <div>
            <span className="block text-xs font-bold text-orange-100 uppercase tracking-wider mb-1">
              आज की प्राथमिकता साधना ({profile.dominantKarma} कर्म शमन)
            </span>
            <span className="font-medium text-sm sm:text-base leading-snug block">
              {remedies.primarySadhana}
            </span>
          </div>
        </div>
