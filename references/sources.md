...
| TIRTHANKARA-NAK-VERIFY | Verification of the 24 Tirthankara birth-nakshatra table against the published record | `references/extracted/TIRTHANKARA_NAKSHATRA__verification.md` | derived | 19 of 24 birth nakshatras confirmed; pins `check:tirthankara`. Secondary — Tiloyapannatti ch.4 wins on conflict. |
| NAKSHATRA-3WAY | Three-way reconciliation: engine vs the repo's own authoring files vs the published record | `references/extracted/NAKSHATRA_SOURCES__three_way_analysis.md` | derived | Establishes that `tirthankar_data.md` and `JAIN NAKSHATRA RULING FRAMEWORK.md` are NOT authoritative for birth nakshatra; resolves #19 Mallinatha; shows the param_shubha rule was never implemented. |
